...
function curl($url,$method,$params,$auth){
    ...
    // 'Authorization: WECHATPAY2-SHA256-RSA2048 mchid:"1606638695",serial_no:"asdsdssas",serial_no:"aasdasd",timestamp:'.time.',signature:"dasdad"'
...
/**
 * pdfotter下载pdf文件
 * @param string pdf_templates pdf文件id
 * @param array params pdf文件变量
 * @param string file 文件保存路径
 * @param string apikey api秘钥
 *
 *  $pdf_templates='tem_pVXn57n3MkVaY5';
    $params=['title'=>'测试标题111'];
    $file='./123.pdf';
    $res=pdf_curl($pdf_templates,$params,$file);
    return $res;
 * */
function pdf_curl($pdf_templates,$params,$file,$apikey='prod_WjF37Sd5xZm3LXkVaKdXraCdCf8CEWjE'){
    $curl = curl_init();
    $url='https://www.pdfotter.com/api/v1/pdf_templates/'.$pdf_templates.'/fill';
    curl_setopt($curl, CURLOPT_URL, $url);//设置请求的URL
    curl_setopt($curl, CURLOPT_RETURNTRANSFER,1); //设为TRUE把curl_exec()结果转化为字串，而不是直接输出
    curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, FALSE);    // https请求时要设置为false 不验证证书和hosts  FALSE 禁止 cURL 验证对等证书（peer's certificate）, 自cURL 7.10开始默认为 TRUE。从 cURL 7.10开始默认绑定安装。
    curl_setopt($curl, CURLOPT_SSL_VERIFYHOST, FALSE);//检查服务器SSL证书中是否存在一个公用名(common name)。
    ...
    curl_setopt ( $curl, CURLOPT_HTTPHEADER, $header);//设置 HTTP 头字段的数组。格式： array('Content-type: text/plain', 'Content-length: 100')
    //请求时间
    curl_setopt ($curl, CURLOPT_CONNECTTIMEOUT, 30);//设置连接等待时间
    if(is_array($params)){
        $params=['data'=>$params];
