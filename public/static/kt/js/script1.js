var goodsList = [{
		...
		goodsInfo: '得力USB插头智能有线/无线插座转换器充电魔方 多功能家用插排接线板 无线 3组合孔',
		price: 199,
		goodsCount: 1,
...
function loadGoods() {
	$.each(goodsList, function(i, item) {
		var goodsHtml = '<div class="goods-item">' +
			...
			'<div class="col-md-1 col-xs-1 car-goods-info">' +
			'<label><input type="checkbox" class="goods-list-item"/></label>' +
			'</div>' +
			'<div class="col-md-5 col-xs-5 car-goods-info goods-image-column gwc-jieshao " style="display:block;"> ' +
			'<img class="goods-image" src="' + item.imgUrl + '" style="width: 80px; height: 80px;" />' +
			'<p class="" id="goods-info">' +
			item.goodsInfo +
			'</p>' +
			'</div>' +
			'<div class="col-md-1 col-xs-1 car-goods-info goods-price"><span class="hidden-xs">￥</span><span class="single-price hidden-xs">' + item.price + '</span></div>' +
			'<div class="col-md-1 col-xs-1 car-goods-info goods-counts">' +
			'<div class="input-group">' +
			'<div class="input-group-btn">' +
			'<button type="button" class="btn btn-default2 car-decrease">-</button>' +
			'</div>' +
			'<input type="text" class="form-control goods-count" value="' + item.goodsCount + '">' +
			'<div class="input-group-btn">' +
			'<button type="button" class="btn btn-default2 car-add">+</button>' +
			'</div>' +
			'</div>' +
			'</div>' +
			'<div class="col-md-1 col-xs-1 car-goods-info goods-money-count"><span></span><span class="single-total">￥' + item.singleGoodsMoney + '</span></div>' +
			'<div class="col-md-3 col-xs-3 car-goods-info goods-operate">' +
			'<button type="button" class="btn2 btn-danger item-delete">删除</button>' +
			'</div>' +
			'</div>' +
			'</div>' +
			'</div>'
